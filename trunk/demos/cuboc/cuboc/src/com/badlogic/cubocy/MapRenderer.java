...
public class MapRenderer {	
	...
	private void createAnimations() {
		...
		//replace bob to new Image
		this.idleLeftTexture = new Texture(Gdx.files.internal("data/idle_left.png"));
		TextureRegion[] idleLeftRegion = new TextureRegion(idleLeftTexture).split(42, 42)[0];
		
		TextureRegion[] idleRightRegion = new TextureRegion(idleLeftTexture).split(42, 42)[0];
		for(TextureRegion region:idleRightRegion) region.flip(true, false);
		
		this.runLeftTexture = new Texture(Gdx.files.internal("data/run_left.png"));
		TextureRegion[] runLeftRegion = new TextureRegion(runLeftTexture).split(42, 42)[0];
		
		TextureRegion[] runRightRegion = new TextureRegion(runLeftTexture).split(42, 42)[0];
		for(TextureRegion region:runRightRegion) region.flip(true, false);
		...
		bobRight = new Animation(0.1f,runRightRegion);
		bobLeft = new Animation(0.1f, runLeftRegion);
		bobIdleRight = new Animation(0.1f,idleRightRegion);
		bobIdleLeft = new Animation(0.1f,idleLeftRegion);
		bobDead = new Animation(0.1f,idleLeftRegion[2]);
		...
		spawn = new Animation(0.1f,new TextureRegion(idleLeftRegion[2],0,0,42,16),
				new TextureRegion(idleLeftRegion[2],0,0,42,32),
				new TextureRegion(idleLeftRegion[2],0,0,42,48),
				new TextureRegion(idleLeftRegion[2],0,0,42,64));
		dying = new Animation(0.1f,new TextureRegion(idleLeftRegion[2],0,0,42,64),
				new TextureRegion(idleLeftRegion[2],0,0,42,48),
				new TextureRegion(idleLeftRegion[2],0,0,42,32),
				new TextureRegion(idleLeftRegion[2],0,0,42,16));
		dispenser = split[5];
	...
	private void renderBob() {
		...
		batch.draw(anim.getKeyFrame(map.bob.stateTime, loop), map.bob.pos.x, map.bob.pos.y, 1.5f, 1.5f);
	}
